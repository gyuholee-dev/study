...
SELECT [테이블A].[필드], [테이블B].[필드] FROM [테이블A]
LEFT JOIN [테이블B] ON [테이블A].[필드] = [테이블B].[필드]; --맞지 않아도 조인
...
CREATE TABLE [테이블]
  (
    ...
    [필드명] INT, -- 정수
    [필드명] INT AUTO_INCREMENT, -- 정수, 자동 증가. 자동증가일 경우 인서트하지 않는다.
...
-- 레코드 인서트
INSERT INTO [테이블]([필드명],[필드명],[필드명]) -- 레코드 값이 같은순서 같은갯수일 경우 필드명 생략가능
  VALUES ([레코드],[레코드],[레코드]);
-- 레코드 업데이트
UPDATE [테이블]
  SET [필드명]=[레코드],
